...
# Init
#
data_folder = 'output/bnb_eur_keep_value'
...
strategy = ttm.strategy.KeepValue(
	pair='BNB/EUR',
	initial_target_value=111.4,  # EUR
...
logger = ttm.logger.Multi(
	...
	ttm.logger.Telegram(
		token=keyring.get_password('telegram', 'bnbeurToken'),         # secures the Telegram bot
		password=keyring.get_password('telegram', 'chatbotPassword'),  # secures the TTM bot
		root_folder=data_folder,
	),
)
logger.set_pair('BNB/EUR')
